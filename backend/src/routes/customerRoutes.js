...
router.post('/reset-password', async (req, res) => {
  try {
    ...
    if (!phone || !newPassword) {
      return res.status(400).json({ success: false, message: 'Telefon ve yeni şifre zorunludur' });
    }
    ...
    if (!customer) {
      return res.status(404).json({ success: false, message: 'Bu telefon numarasına ait hesap bulunamadı' });
    }
